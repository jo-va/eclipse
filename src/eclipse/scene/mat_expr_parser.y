...
%type <NValue>          f_or_mat f_or_tex f3_or_tex f3
...
param       : REFLECTANCE   ':' f3_or_tex   { $$ = { REFLECTANCE, $3 }; }
            ...
            | SCALE         ':' NUM         { $$ = { SCALE, NValue::num($3) }; }
            | ROUGHNESS     ':' f_or_tex    { $$ = { ROUGHNESS, $3 }; }
            ;

f_or_mat    : NUM                           { $$ = NValue::num($1); }
            | MAT                           { $$ = NValue::known_ior(unquote($1)); }
            ;

f_or_tex    : NUM                           { $$ = NValue::num($1); }
            | TEX                           { $$ = NValue::texture(unquote($1)); }
            ;
...
f3          : '{' NUM ',' NUM ',' NUM '}'   { $$ = NValue::vec3($2, $4, $6); }
            ;

bxdf_op     : MIX  '(' bxdf_any ',' bxdf_any ',' NUM ')' { $$ = new NMix($3, $5, $7); }
            | MMAP '(' bxdf_any ',' bxdf_any ',' TEX ')' { $$ = new NMixMap($3, $5, NValue::texture(unquote($7))); }
            | BMAP '(' bxdf_any ',' TEX ')'              { $$ = new NBumpMap($3, NValue::texture(unquote($5))); }
            | NMAP '(' bxdf_any ',' TEX ')'              { $$ = new NNormalMap($3, NValue::texture(unquote($5))); }
